...
## ⚠️ 仕様と注意事項 (Important Notes)

本アプリケーションはフロントエンド完結型で動作し、データはすべてブラウザの「ローカルストレージ（Local Storage）」に保存される仕様となっています。そのため、以下の点にご注意ください。

- **データの消失:** ブラウザの履歴、キャッシュ、またはサイトデータをクリアすると、登録した在庫データはすべて消失します。
- **シークレットモードでの動作:** シークレットモード（プライベートブラウズ）で利用した場合、ウィンドウを閉じたタイミングでデータがリセットされます。
- **端末間の同期不可:** データはご利用のブラウザ内にのみ保存されるため、別のブラウザや端末（PCとスマートフォン間など）でデータを共有・同期することはできません。

※本格的な運用や複数人でのデータ共有を行う場合は、別途バックエンドサーバーおよびデータベース（PostgreSQLなど）との連携が必要です。
